...
public class RegisteredUserControllerIntegrationTest {
	...
	public void testFindByEmail() {
		...
		ResponseEntity<UserDTO> responseEntity =
				restTemplate.exchange("/registeredUser/byEmail/a4@a", HttpMethod.GET, new HttpEntity<>(headers),new ParameterizedTypeReference<UserDTO>() {
				});
		
		UserDTO found = responseEntity.getBody();
		assertEquals(HttpStatus.OK, responseEntity.getStatusCode());
		assertEquals(1L, found.getId());
		assertEquals("a4@a", found.getEmail());
	}
	...
	public void testSubscribeUnsubscribe() {
		...
		int userSize = this.userService.findOneByEmail("a4@a").getCulturalOffers().size();
		...
		RegisteredUser find = this.userService.findOneByEmail("a4@a");
		...
		assertEquals(HttpStatus.OK, responseEntity.getStatusCode());
		assertEquals(userSize, this.userService.findOneByEmail("a4@a").getCulturalOffers().size());
